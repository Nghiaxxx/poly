...
export default function NewsDetailComponent({ newsId }: { newsId: string }) {
  ...
  // Build safe absolute image URL (handles http(s), absolute, or relative paths)
  const imageSrc = newsDetail.hinh?.startsWith('http')
    ? newsDetail.hinh
    : `${getBaseUrl()}${newsDetail.hinh?.startsWith('/') ? '' : '/'}${newsDetail.hinh || ''}`;

  const safeContent = newsDetail.noi_dung || '';
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            "publisher": {
              ...
              "logo": {
                "@type": "ImageObject",
                "url": "https://polysmart.nghiaht.io.vn/logo.png"
              }
            },
            "mainEntityOfPage": {
              "@type": "WebPage",
              "@id": `https://polysmart.nghiaht.io.vn/news/${newsDetail.id_danh_muc?._id || ''}/${newsDetail._id}`
            }
          })
        }}
      />
